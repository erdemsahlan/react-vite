...
import { getAllProductMovements, createProductMovement } from './productTradeApi';
import type { ProductInOrOutDto, AlisSatis, ParaTipi } from './productTradeApi';
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <main style={{ flex: 1, padding: 32, background: '#f4f6fb' }}>
        <h1 style={{ fontSize: 36, fontWeight: 700, marginBottom: 24, textAlign: 'center' }}>Ürünler Alış-Satış</h1>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ textAlign: 'left', padding: 8, borderBottom: '1px solid #eee', width: 80 }}>Aktif</th>
              ...
              <tbody>
                {movements.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td style={{ padding: 8, borderBottom: '1px solid #f4f6fb', width: 100 }}>{m.alisSatis === 'ALIS' ? 'Alış' : 'Satış'}</td>
                    ...
                    <td style={{ padding: 8, borderBottom: '1px solid #f4f6fb', width: 100 }}>{m.paraTipi === 'TURKLIRASI' ? 'TL' : m.paraTipi}</td>
                    <td style={{ padding: 8, borderBottom: '1px solid #f4f6fb', width: 100 }}>{m.dovizKuru}</td>
                    <td style={{ padding: 8, borderBottom: '1px solid #f4f6fb', width: 80 }}>{m.active ? 'Aktif' : 'Pasif'}</td>
                    <td style={{ padding: 8, borderBottom: '1px solid #f4f6fb', width: 120, textAlign: 'right' }}>
                      <Button 
                        variant="outline" 
                        onClick={() => alert(`Detay: ${m.id}`)}
            ...
            <div style={{ background: '#fff', borderRadius: 8, padding: 32, minWidth: 400, boxShadow: '0 2px 16px #0002', position: 'relative' }}>
              ...
              <form onSubmit={handleCreate}>
                ...
                <div style={{ marginBottom: 18 }}>
                  ...
                  <select id="alisSatis" name="alisSatis" value={form.alisSatis} onChange={handleInputChange} disabled style={{ width: '100%', padding: 8, borderRadius: 4, border: '1px solid #ccc', backgroundColor: '#f5f5f5' }}>
                ...
                <div style={{ marginBottom: 18 }}>
                  ...
                  <select id="paraTipi" name="paraTipi" value={form.paraTipi} onChange={handleInputChange} style={{ width: '100%', padding: 8, borderRadius: 4, border: '1px solid #ccc' }}>
                    <option value="TURKLIRASI">Türk Lirası</option>
